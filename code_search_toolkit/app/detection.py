...
def run_ccstokener(dataset_path: str, params: dict, result_folder: str, language: str, worker_id: int, save_logs: bool):
    ...
    try:
        output = subprocess.check_output(cmd, universal_newlines=True)
        if "Error" in output or "ERROR" in output or \
            "Exception" in output or "EXCEPTION" in output:
            raise RuntimeError("find error or exception in output logs")
...
def run_sptokener(dataset_path: str, params: dict, result_folder: str, language: str, worker_id: int, save_logs: bool):
    script_path = os.path.join(os.getcwd(), "tools", "run_SPtokener.sh")
    cmd = [script_path, dataset_path, result_folder, language, str(worker_id)]
    ...
    try:
        output = subprocess.check_output(cmd, universal_newlines=True)
        if save_logs:
            with open(os.path.join(result_folder, MethodEnum.SPTOKENER.value, "logs.txt"), "w+") as f:
                f.write(output)
    except (subprocess.CalledProcessError, RuntimeError) as e:
        if save_logs:
            with open(os.path.join(result_folder, MethodEnum.SPTOKENER.value, "logs.txt"), "w+") as f:
                f.write(output)
                f.write(f"\nERROR: {e}")
        raise RuntimeError(f"SPtokener error: {e}")
