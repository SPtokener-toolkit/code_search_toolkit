...
ALLOWED_LANGUAGES_FOR_METHOD = {
    ...
    MethodEnum.SPTOKENER: [LanguageEnum.JAVA],
}
...
async def create_search_task(
    ...
    for m in methods_list:
        ...
        for param_name, meta in params_meta.items():
            ...
            expected_type = int if meta['type'] == 'int' else str if meta['type'] == 'str' else float
            try:
...
def process_search_task(
    ...
    def run_method(method: dict):
        ...
        if name.upper() == MethodEnum.NIL_FORK.value.upper():
        ...
        elif name.upper() == MethodEnum.SPTOKENER.value.upper():
            run_sptokener(
                dataset_folder,
                params,
                results_folder,
            ...
            return MethodEnum.SPTOKENER.value, os.path.join(results_folder, MethodEnum.SPTOKENER.value)
