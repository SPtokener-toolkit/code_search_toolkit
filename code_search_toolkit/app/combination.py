...
def combine_results(results, combination, query_file, results_folder):
    try:
        ...
        if MethodEnum.SPTOKENER.value in results:
            method_results[MethodEnum.SPTOKENER.value] = read_sptokener_results(results[MethodEnum.SPTOKENER.value], query_file)
            print(f"DEBUG: SPtokener results len = {len(method_results[MethodEnum.SPTOKENER.value])}")
...
def read_sptokener_results(result_dir, query_file):
    begin_of_path = "/data/input/"
    clones = []
    try:
        filepath = os.path.join(result_dir, 'clonepairs.txt')
        ...
        logging.warning(f"Error reading SPtokener results: {str(e)}")
    return clones
